...
5. **Affordability gate.** `planVideo` reads the balance and compares it with
   ...
   quality: a cheaper render is a different render than the one asked for, and a
   substitution that looks plausible is exactly how a real failure goes unnoticed.
...
| 15 | The same gate **downgraded** to 360p when the balance would not cover 720p, on the reasoning that a cheaper render beats none | The downgrade is gone. An unaffordable request is refused immediately, and a caller who wants 360p asks for it. The justification given at the time — that `abra_t2v_4s_360p` never renders — turned out to be wrong; the refusal stands on the simpler ground that substituting a quality is a different render | `TestDecideVideoPlanRefusesRatherThanDowngrading` |
| 17 | `loadJar` fell back to `CookieDir()/cookies.json` while the bridge wrote `DataDir()/cookies.json` — two files, so the persisted copy was never refreshed by the bridge. Measured 8.5 hours apart. With the browser attached the live sync hides it; without one the engine loaded a stale jar and every call answered 401 | Both are tried, synced file first. A stale snapshot can still 401 once the rotating cookies age out — the CLI has no browser to re-sync from — but the error is clear rather than a wrong result | verified live: the CLI's fallback now reports `data/cookies.json` and the correct account |
...
| 17 | Account selection moved a request to a richer account **without checking whether the request referenced an asset**. Projects are per-account, so a `start_image` id belongs to the project of the account that supplied it — and moving left it pointing at a project the new account cannot see. Measured: an image generated on one account was used as the start frame of a request the engine moved to afford, and resolution retried its full window before failing with "not in the project listing" | `canMoveAccounts` — a request conditioned on `start_image` / `end_image` is refused rather than moved. A request with no conditioning carries no project-bound id and can be served anywhere | `TestCanMoveAccountsRefusesAConditionedRequest` |
| 18 | `RPCIDProjectList` was recorded as `WuwhI`, which is the generation-status RPC. Nothing validated the label, so the id was "known" while the listing was never actually called — and the absence of a listing read as "the API cannot list projects", which kept an editor tab the only way to learn a project id | `UpteDb` with `["projects/*",21,null,null,null,null,[1]]` returns the account's projects. The engine now prefers it over the browser, so a boot navigates nothing | `TestParseProjectListReadsBothRowShapes`, `TestChooseProjectIDPrefersTheListingOverTheBrowser`; verified live: `2 project(s) listed over the transport` with no editor navigation |
| 19 | The project-list parser indexed `row[0]` on each row to classify it, so an account with **no** projects — which answers with an empty inner array — panicked the engine at boot | The length is checked before the index. The empty-account case is now a test rather than a crash | `TestParseProjectListAcceptsAnEmptyAccount` |
