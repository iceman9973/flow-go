...
//
// Absorbed from github.com/kodelyx/Browser-cdp/cdp-control (its `bridge`
// package, v0.0.0-20260920054928-9aaab4e4a4f7) on 2026-09-22, so flow-go no
// longer depends on that module. Change it in place: there is no upstream to
// track and no replace directive to restore.
package bridge
...
// EnsureToken is a no-op kept for backwards compatibility.
// The bridge is completely tokenless.
func (b *Bridge) EnsureToken() error {
	return nil
}

// claim is a no-op kept for backwards compatibility.
func (b *Bridge) claim() {}
...
func (b *Bridge) Listen(ctx context.Context) error {
	...
	log.Printf("bridge: listening for a browser extension on ws://%s (tokenless)", addr)
